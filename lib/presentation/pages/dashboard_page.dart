...
class _DashboardPageState extends State<DashboardPage> {
  ...
  DateTime _cutoffDate(CycleType f) {
    ...
    switch (f) {
      case CycleType.oneWeek:    return now.subtract(const Duration(days: 7));
      case CycleType.thirtyDays: return now.subtract(const Duration(days: 30));
      case CycleType.all:        return DateTime.fromMillisecondsSinceEpoch(0);
    }
  }
  ...
  List<TransactionModel> getFilteredTransactions(List<TransactionModel> allTx){
    ...
    return allTx.where((tx){
      final matchSearch = tx.description.toLowerCase()
        ...
        builder: (ctx, setModalState) {
          ...
          Widget buildCycleChip(CycleType cycle){
            ...
            return ChoiceChip(
              ...
              selected: isSel,
              onSelected: (_) =>
                  setModalState(() => tempSelectedCycle = cycle),
